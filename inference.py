...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for eliVar in eliminationOrder:
            factors1=joinFactorsByVariable(factors, eliVar)
            factors=factors1[0]+[factors1[1]]
            eliFactors=[]
            for i in factors:
                if eliVar in i.unconditionedVariables() and len(i.unconditionedVariables()) !=1:
                    eliFactors.append(eliminate(i,eliVar))
                elif eliVar in i.unconditionedVariables() and len(i.unconditionedVariables())==1:
                    pass
                else:
                    eliFactors.append(i)
            factors=eliFactors
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        newBelief={}
        for p, belief in self.beliefs.items():
            newPosDist=self.getPositionDistribution(gameState, p)
            for p1, belief1 in newPosDist.items():
                newBelief[p1]=newBelief.get(p1,0)+belief1*belief
        self.beliefs=DiscreteDistribution(newBelief)
        self.beliefs.normalize()
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            prob[particle]=prob.get(particle,0)+1
        prob.normalize()
        return prob
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        newProbDict=DiscreteDistribution()
        for particle in self.particles:
            obsProb=self.getObservationProb(observation,\
                                            gameState.getPacmanPosition(),\
                                            particle,\
                                            self.getJailPosition())
            newProbDict[particle]=newProbDict.get(particle,0)+obsProb
        newProbDict.normalize()
        if max(newProbDict.values())==0:
            self.initializeUniformly(gameState)
        else:
            newParticles=[]
            cWeight=0
            num=self.numParticles
            cumulateD=0
            for particle, weight in newProbDict.items():
                cWeight+=weight
                newD=int(num*cWeight)
                for _ in range(newD-cumulateD):
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            newPos.append(newPosDist.sample())
